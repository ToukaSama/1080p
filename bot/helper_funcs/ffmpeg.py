...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg, custom_filename=None):
    ...
    audio_b.append("40k")
    file_genertor_command = f"ffmpeg -hide_banner -loglevel quiet -progress '{progress}' -i '{video_file}' -metadata 'title=Anime Sovereign' -c:v {codec[0]} -map 0 -crf {crf[0]} -c:s copy -pix_fmt yuv420p -s {resolution[0]} -b:v 500k -c:a libopus -b:a {audio_b[0]} -preset {preset[0]} -metadata:s:v 'title=Anime Sovereign' -metadata:s:a 'title=Anime Sovereign' -metadata:s:s 'title=Anime Sovereign' -vf 'drawtext=fontfile=font.ttf:fontsize=30:fontcolor=white@0.5:x=10:y=10:text=Anime Sovereign' '{out_put_file_name}' -y"
